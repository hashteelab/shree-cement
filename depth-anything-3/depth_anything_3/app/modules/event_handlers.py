...
class EventHandlers:
    ...
    def gradio_demo(
        ...
        # Returning None for 3D viewer (not supported without pose data)
        glbfile = None
    ...
    def update_visualization(
        ...
        """
        Reload saved predictions from npz for metric depth visualization.
        Note: 3D GLB viewer is not supported for metric models without pose data.

        Args:
            target_dir: Directory containing results
            show_cam: Whether to show camera (unused, kept for compatibility)
            is_example: Whether this is an example scene
            filter_black_bg: Whether to filter black background (unused)
            filter_white_bg: Whether to filter white background (unused)
            process_res_method: Method for resizing input images (unused)

        Returns:
            Tuple of (glb_file (None), log_message)
        """
        if not target_dir or target_dir == "None" or not os.path.isdir(target_dir):
            return (
                None,
        ...
        return (
            None,  # No GLB file for metric models without pose data
            "Visualization loaded. Use the 'Metric Depth' tab for distance measurement.",
        )
